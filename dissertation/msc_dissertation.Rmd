...
# 
dat_models_specifications = data.frame(fread(here::here("data", "4_model_outcome_data_specifications.csv")))
...
## order covariate effects low to high, based on the 4_covariate_effect_data.csv dataset
impact_df = dat_covariates[order(dat_covariates$estimate_oddsratio), ]
...
top = analysed_specifications %>% 
  ...
  labs(title = "Specification curve", subtitle = "N = 1,000; 95% CI") +
  theme_classic() +
...
vibration_of_effect = dat_models %>%
  ...
  labs(title = "Results space defined through covariate specification", subtitle = "N = 1,000; Odds ratio (OR)") +
  coord_flip() +
  theme_classic() +
...
covariate_effects = dat_covariates %>%
  ...
  labs(title = "Specified covariate effects", subtitle = "N = 1,000; 95% CI", x = "Covariates") +
  theme_classic() +
  theme(panel.grid.major.y = element_line(colour = "grey"),
