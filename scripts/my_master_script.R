...
requiredPackages = c("psych", "plotrix", "rstudioapi", "DescTools", "here", "data.table", "lme4")
...
# Full variable names
variables = c("Name", "Height (in cm)", "Weight (in kg)", "Age (in years)", "Specific position", "Club", "League country", "Number of games", "Number of goals", "Number of victories", "Yellow cards", "Yellow/red cards", "Red cards", "all_reds",  "Referee ID", "Referee country", "Referee number of games", "Skin tone (as numerical)", "Skin tone (binary)", "Implicit bias", "Explicit bias")

# Dependent variables
dv = c("all_reds", "red_cards", "yellow_red_cards", "yellow_cards")

# Independent variables
iv = c("player", "height_cm", "weight_kg", "age_yrs", "specific_pos", "club", "league_country", "games", "goals", "victories", "ref", "ref_country", "ref_games", "skin_tone_num", "no_white", "imp_bias", "exp_bias") 
...
all_models = apply(specifications, 1, function(x) paste(c("all_reds ~ 1", iv[x]), collapse = " + "))

# run all models
all_models_results = mclapply(all_models, function(x) glm(x, data = dat, family = "binomial"))
